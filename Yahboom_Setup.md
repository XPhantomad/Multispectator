...
## Container Structure of the Yahboom

![alt text](containerSC.png)
1. rosmaster-m3pro:1.0.10 - rosmaster-m3pro (name fixed):
    - container for user applications
    - usually not started (docker start rosma... + exec_m3pro)
    - right click to "Attach VS-Code" for full access
2. rosmaster-m3pro:1.0.3 - youthful_germain (name changes):
    - container which automatically starts the handle control 
3. micro-ros-agent:humble - stupefied_lewin (name changes):
    - micro-ros container which maintains the initial YB-Node for the chassis control (movement, arm, gripper, ...)
    - restart, if topics did not appear in the list 
...
- if not, restart the Containers 2. and 3. (see above)
...
## 5. Control the robot movement with ROS2 (/cmd_vel)
- ros2 topic pub /cmd_vel geometry_msgs/msg/Twist "{linear: {x: -0.1, y: -0.1, z: 0.0}, angular: {x: 0.0, y: 0.0, z: 0.0}}" -r 10
- linear y lets the robot strafe (sideward movement)
- **IMPORTANT: Robot drives very unsmooth when the controller handle is active too, otherwise it drives way faster and smooth!!!**

## 6. ROS Workspace Preparation for MultiSpectator
...
## 7. Start MultiSpectator - SingleRobotController (deprecated)
...
## 8. Use Camera and Arm for AprilTag detection and following (from the tutorial page)
...
- Predefined package works okay (Desktop tracking and gripping machine code):
    ...
    - robot follows Apriltag object - if it stays still, it beeps - now, grasp mode can be triggered with "space" - to enter follow mode again press "m"
...
- Set all joints to a testable level (**IMPORTANT: do not set all to 0!!!!**)
    ...
    - time: 3000 for slow movement to preserve servos
    - Pose close to initial pose: 
...
## 9. Camera Controller startup: (deprecated)
...
## 10. Setup with Launch Script 
- multispectator.launch.py must be located in M3Pro_ws/src/m3pro_bringup/launch
- locate required config files in the config folder beside it (ekf.yaml, laserscan_merge.yaml, slam_params.yaml)
- build: ```colcon build --packages-select m3pro_bringup```
- test: ```ros2 launch m3pro_bringup multispectator.py```
...
- user mode --> specifiy goal or joystick input (has to consider camera limits and should rotate the robot in this case!!!!)
...
- manual mode: 
    - activate in the webapp 
    - then activate the handle control with R2 on the handle
    - deactivate it afterwards

## Improve Positioning:
1. **Resart the micro-ros container everytime the robot restarts at its initial position --> clean odom values !!**
...
- second approach with laser improvement
